# Outputs a simple (ugly) HTML page listing all faculty achievements,
# unless you specify a department filter.
# 
# Call this with a URL like
#
# http://server:8080/Intranet/role-based-content/faculty/achievements/achievements-for-publication-in-endeavors-magazine/pullachievements_html?deptfilter=Writing%20Center
#
# and note that you can append
#
# ?deptfilter=Writing Center
#
# if you want it to output just departments matching that exact name

import logging
log = logging.getLogger('pullachievements_genhtml')
...
class Award:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Award.achievement_name,

    def achievementheaderend(self):
        return  "</ul>"
...
class Book:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Book.achievement_name,
...
class Fellowship:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Fellowship.achievement_name,
...
class Presentation:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Presentation.achievement_name,
...
class Exhibition:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Exhibition.achievement_name,
...
class Appointment:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Appointment.achievement_name,
...
class Journal:
    ...
    def achievementheaderstart(self):
        return "<h3>%s</h3><ul>" % Journal.achievement_name,
...
def pullachievements_html(self, deptfilter=''):
    ...
    def output_achievement_type(line, ac_type, acs):
        if acs:
            len_acs = len(acs)
            line += str((acs[0].achievementheaderstart())[0])
            # output the achievements the person has of this type
            for ac in acs:
                line += str(ac)
            line += str(acs[0].achievementheaderend())
        else:
            len_acs = 0
    ...
    depts.sort()
    for dept in depts:
        if deptfilter == '' or deptfilter == dept:
            names = [k for k in alldata_by_dept[dept].keys()]
            names.sort()
            for name in names:
                #line = "|".join([dept, name])
                out.append("<h1>%s</h1>" % dept)
